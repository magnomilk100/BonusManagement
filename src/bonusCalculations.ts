...
/**
 * Creates a function that returns the daily amortization expense for a given date,
 * based on the bonus terms. It supports both straight-line and sum-of-digits methods.
 * @param {BonusFormData} bonus - The bonus data object.
 * @param {Date} contractualAmortizationEndDate - The original end date of amortization.
 * @param {number} bonusAmount - The total amount of the bonus.
 * @returns {(currentDate: Date) => number} A function that takes a date and returns the expense for that day.
 */
export const getDailyExpenseRateCalculator = (bonus: BonusFormData, contractualAmortizationEndDate: Date, bonusAmount: number): ((currentDate: Date) => number) => {
    ...
    if (hasProRataClawback && totalContractualDays > 0) {
        ...
        return (currentDate: Date): number => {
            const daysRemaining = daysBetween(currentDate, contractualAmortizationEndDate);
            return (bonusAmount * Math.max(0, daysRemaining)) / sumOfDigits;
        };
    } else {
        // Straight-Line logic for 'full-refund' or no clawback
        const straightLineDailyRate = totalContractualDays > 0 ? bonusAmount / totalContractualDays : 0;
        return () => straightLineDailyRate;
    }
};
...
export const calculateAmortizationDetails = (bonus: BonusFormData, selectedPeriod?: string): AmortizationDetail | null => {
    ...
    const getExpenseForDay = getDailyExpenseRateCalculator(bonus, contractualAmortizationEndDate, bonusAmount);
    ...
    if (amortizationStartDate <= effectiveAmortizationEndDate) {
        ...
        while (currentDate <= effectiveAmortizationEndDate) {
            const expenseForDay = getExpenseForDay(currentDate);
